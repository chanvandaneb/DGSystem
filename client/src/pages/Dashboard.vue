  ...
  <div class="space-y-6">
    ...
    <div v-if="summary" class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      <Card
        ...
        :key="i"
        :class="`border-0 text-white shadow-md ${accentBg[kpi.accent]}`"
      >
        <CardContent class="px-5 pt-5 pb-4">
          <div class="flex items-start justify-between">
            <div>
              <p class="text-xs uppercase tracking-wider text-white/70">{{ kpi.label }}</p>
              <p class="mt-1.5 text-3xl font-bold">{{ kpi.value }}</p>
              <p class="mt-0.5 text-xs text-white/70">{{ kpi.sub }}</p>
            </div>
            <component :is="kpiIcons[i]" class="h-5 w-5 text-white/40" />
          </div>
          <div class="mt-4 flex items-end justify-between">
            <svg width="80" height="28" class="overflow-visible opacity-60">
              <path :d="sparkPath(kpi.sparkline)" fill="none" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
            </svg>
            <span class="rounded-full bg-white/20 px-2 py-0.5 text-xs font-semibold text-white/90">{{ kpi.delta }}</span>
          </div>
        </CardContent>
      </Card>
      ...
      <Card v-if="summary.taskBreakdown" class="shadow-sm">
        <CardContent class="flex items-center gap-5 px-5 pt-5 pb-4">
    ...
    <div v-if="summary" class="grid gap-4 lg:grid-cols-3">
      ...
      <Card class="lg:col-span-2 shadow-sm">
        <CardHeader class="px-5 pt-5 pb-3">
          ...
          <p class="text-xs text-muted-foreground">Hours per day — red bars = late check-in.</p>
        </CardHeader>
        <CardContent class="px-5 pb-5">
          <div class="flex h-44 items-end gap-2">
            ...
            >
              ...
              <div
                class="w-full max-w-10 rounded-t-md transition-all duration-200"
      ...
      <Card class="shadow-sm">
        ...
        <CardContent class="flex flex-col items-center px-5 pb-5">
          ...
          <div class="mt-4 w-full space-y-2 text-sm">
            <div v-for="[key, count, color] in [['Online', summary.presence.online, 'bg-emerald-500'], ['Idle', summary.presence.idle, 'bg-amber-400'], ['Offline', summary.presence.offline, 'bg-zinc-400']] as [string, number, string][]"
              :key="key" class="flex items-center justify-between">
              <span class="flex items-center gap-2">
                <span :class="['h-2 w-2 rounded-full', color]" />{{ key }}
              </span>
    ...
    <div v-if="summary" class="grid gap-4 lg:grid-cols-2">
      ...
      <Card class="shadow-sm">
        ...
        <CardContent class="max-h-72 space-y-1 overflow-y-auto px-5 pb-5">
          <div
            v-for="item in summary.recentActivity"
            :key="item.id"
            class="flex items-start gap-3 rounded-lg px-2 py-2 transition-colors hover:bg-accent/50"
          >
      ...
      <Card class="shadow-sm">
        ...
        <CardContent class="max-h-72 space-y-1 overflow-y-auto px-5 pb-5">
          <div v-for="u in summary.userStatuses" :key="u.id"
            class="flex items-center justify-between rounded-lg px-2 py-2 transition-colors hover:bg-accent/50">
    ...
    <!-- Announcements -->
    <Card v-if="summary" class="shadow-sm">
      ...
      <CardContent class="grid gap-3 px-5 pb-5 sm:grid-cols-2">
        <div
          v-for="a in summary.recentAnnouncements"
          :key="a.id"
          class="flex cursor-pointer gap-3 rounded-xl border border-border p-3 transition-colors hover:bg-accent/50"
          @click="router.push('/announcements')"
        >
        ...
        <p v-if="!summary.recentAnnouncements.length" class="col-span-2 text-sm text-muted-foreground">No announcements</p>
      </CardContent>
    </Card>
  </div>
</template>
